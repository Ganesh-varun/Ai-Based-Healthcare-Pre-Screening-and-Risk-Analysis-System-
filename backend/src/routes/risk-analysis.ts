...
router.post('/analyze', async (req: Request, res: Response) => {
    try {
        ...
        const ftsQuery = `
            ...
            LIMIT 10;
        `;
        const ftsResult = await pool.query(ftsQuery, [symptoms]);
        ...
        if (topMatch) {
            ...
            if (criticalDiseases.some(d => topName.includes(d))) datasetBonus += 40;
            else if (seriousDiseases.some(d => topName.includes(d))) datasetBonus += 20;
    ...
    - Total: ${totalScore.toFixed(1)}`);
        ...
        if (totalScore >= 35 || isExplicitEmergency) riskLevel = 'emergency';
        else if (totalScore >= 15 || isExplicitHigh) riskLevel = 'high';
        ...
        const addedSymptoms = new Set<string>();
